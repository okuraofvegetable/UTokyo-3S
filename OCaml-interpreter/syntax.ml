...
exception PatternMatchError

let rec find_match pt va =
  match (pt,va) with
  | ((PInt x),(VInt y)) -> (if (x=y) then [] else (raise PatternMatchError))
  | ((PBool x),(VBool y)) -> (if (x=y) then [] else (raise PatternMatchError))
  | ((PPair (p1,p2)),(VPair (v1,v2))) -> ((find_match p1 v1)@(find_match p2 v2))
  | (PNil,VNil) -> []
  | ((PCons(p1,p2)),(VCons(v1,v2))) -> ((find_match p1 v1)@(find_match p2 v2))
  | ((PVar n),v) -> [(n,v)]
  | _ -> (raise PatternMatchError)
...
let rec print_value v =
  ...
  | VFun (n,e,_) -> print_string "<fun>"
  | VRecFun (id,n,e1,e2) -> print_string "<rec fun>"
  | VNil -> print_string "[]"
